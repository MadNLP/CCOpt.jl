######################### Scholtes Relaxation #########################
struct ScholtesMultiRelaxation{T, VT} <: AbstractMPCCRelaxation{T, VT}
...
function ScholtesMultiRelaxation(mpcc::AbstractMPCCModel{T, VT}) where {T, VT}
    if !is_vertical(mpcc)
    ...
    return ScholtesMultiRelaxation(mpcc, meta, σ, σopt, δ1, δ1opt, δ2, δ2opt)
end

# Counters should be forwarded
function Base.getproperty(rnlp::ScholtesMultiRelaxation, sym::Symbol)
    if sym ∈ [:counters]
...
######################### NLPModels Callbacks #########################
function NLPModels.obj(rnlp::ScholtesMultiRelaxation, x::AbstractVector)
    return NLPModels.obj(rnlp.mpcc, x)
end

function NLPModels.grad!(
    rnlp::ScholtesMultiRelaxation,
    x::AbstractVector,
    gx::AbstractVector,
)
...
function NLPModels.hess_coord!(
    rnlp::ScholtesMultiRelaxation{T, VT},
    x::AbstractVector{T},
    y::AbstractVector{T},
    H::AbstractVector{T};
...
function initialize_relaxation(rnlp::ScholtesMultiRelaxation{T}, σ::T, δ::T) where {T}
    rnlp.σ .= σ
    rnlp.σopt .= σ
    rnlp.δ1 .= δ
    rnlp.δ1opt .= δ
...
function get_log_relaxation(rnlp::ScholtesMultiRelaxation{T}) where {T}
    return log(10, mapreduce((a, b, c) -> min(a, b, c), max, rnlp.σ, rnlp.δ1, rnlp.δ2))
end
