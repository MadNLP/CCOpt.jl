...
function crossover(
    ...
    while ii <= max_iter
        while true
            ...
            success, lpcc_sol, y_next = solve_lpcc(lpcc, lpcc_solver_opts;lpcc_solver_kwargs...)
            if norm(lpcc_sol.solution) <= stationarity_tol || f_curr - lpcc_sol.objective <= stationarity_tol
